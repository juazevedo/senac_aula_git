<?php 
	...
	function incluirComentario($dataPublicacao, $ip, $texto, $idUsuario) {
		$cnn = abrirConexao();
		$dataPublicacao = mysqli_real_escape_string($cnn, $dataPublicacao);
		$ip = mysqli_real_escape_string($cnn, $ip);
		$texto = mysqli_real_escape_string($cnn, $texto);
		$idUsuario = mysqli_real_escape_string($cnn, $idUsuario);
		$id = incluir($cnn, "INSERT INTO comentarios VALUES (NULL,'$dataPublicacao', '$ip', '$texto', $idUsuario)");
		fecharConexao($cnn);
		return $id;
	}
	
	function alterarComentario($id, $dataPublicacao, $ip, $texto, $idUsuario) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$dataPublicacao = mysqli_real_escape_string($cnn, $dataPublicacao);
		$ip = mysqli_real_escape_string($cnn, $ip);
		$texto = mysqli_real_escape_string($cnn, $texto);
		$idUsuario = mysqli_real_escape_string($cnn, $idUsuario);
		$con = alterarOuExcluir($cnn, "UPDATE comentarios SET data_publicacao='$dataPublicacao', ip='$ip', texto='$texto', usuario_id=$idUsuario WHERE id=$id");
		fecharConexao($cnn);
		return $con;
	}
	function excluirComentario($id) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$con = alterarOuExcluir($cnn, "DELETE FROM comentarios WHERE id = $id");
		fecharConexao($cnn);
		return $con;
	}
	function consultarComentario($id) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$arr = consultar($cnn, "SELECT c.*, u.nome AS nome_usuario FROM comentarios c INNER JOIN usuario u ON u.id = c.usuario_id WHERE c.id = $id");
		fecharConexao($cnn);
		return $arr;
	}
	function listarComentario($idUsuario, $ativo) {
		$cnn = abrirConexao();
		
		$sql = "SELECT c.*, u.nome AS nome_usuario FROM comentarios c INNER JOIN usuario u ON u.id = c.usuario_id WHERE 1 = 1 ";
		
		if(!is_null($idUsuario)) {
			$idUsuario = mysqli_real_escape_string($cnn, $idUsuario);
			$sql .= " AND c.usuario_id = $idUsuario";
		}
		$arr = listar($cnn, $sql);
		fecharConexao($cnn);
		return $arr;
